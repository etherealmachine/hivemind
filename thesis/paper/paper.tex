...
% TODO 7
% comment: how did you come with the constant of 50?
% answer: balancing exploration versus computational requirements and preliminary results
% also, this was suggested as a good starting point in the computer Go mailing list (5, 10, and 50 were some common values)
...
% TODO 9
% comment: would it be possible to include patterns/weights from competition-level programs such as MoGo (in Go)?
% answer: yes
...
\item The algorithm iterates the following with each iteration forming one ``generation'':
	...
	% TODO 10
	% comment: this appears to be against the spirit of natural evolution where mutation is always (presumably) present
	% answer: 5/35 children will be unmutated, so mutation is definitely still present in the system.
	% This is also standard in a lot the genetic algorithm literature.
...
chosen as opponents.
% TODO 12
% comment: so the pool consisted of three basic policies? Why wasn't MoHex a part of the pool? In fact, the authors touch on this later in the paper:
% "A potential problem with self-learning is that the system might only be learning how to exploit the weaknesses of its competitors, and not making
% a general improvement in playing strength." This sentence is also problematic. Namely, it is unclear what a "general improvement in playing strength" is. A strong player against player A may be weak against player B. It is unclear how to define an opponent-independent measure of playing
% strength. However, by not including MoHex in the pool during the evolution the authors did not specifically evolve their continuation policies
% against MoHex. I wonder why.
% answer: To preserve MoHex as an outside comparator. Learning from self-play is also the research question.
...
% TODO 13
% comment: yes, I think the results of this paper would be even more interesting if either (i) only continuation policies were compared
% e.g. by disabling all other techniques in MoHex) or (ii) other advanced techniques (e.g., child-selection policy, the update policy, etc.)
% were added to Hivemind as well.
% answer: Comparing just continuation policies is a tricky issue. Hivemind specifically scores candidates on winning a full game using MCTS, it does
% not attempt to build a strong continuation policy. It is true that a strong continuation policy can lead to stronger overall play, but especially
% the simulation balancing paper shows that strong playouts are not always the best approach to a stronger overall player.
% As to (ii), improving Hivemind's other areas is definitely a good idea
